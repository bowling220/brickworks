...
export function EnvironmentSettingsDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="w-[calc(100vw-24px)] sm:max-w-2xl max-h-[88dvh] flex flex-col bg-slate-900/95 border-slate-700/80 text-white backdrop-blur-xl shadow-2xl p-0 overflow-hidden">
        {/* Header */}
        <DialogHeader className="p-4 sm:p-6 pb-4 border-b border-slate-800 bg-slate-900/60 shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <button
              ...
              className="self-start sm:self-auto text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 hover:text-white transition-colors border border-slate-700"
          ...
          {/* Navigation Tabs */}
          <div className="flex gap-2 mt-4 pt-2 border-t border-slate-800/80 overflow-x-auto pb-1 custom-scrollbar shrink-0">
            <button
              onClick={() => {
                getAudioManager().playUiClick();
